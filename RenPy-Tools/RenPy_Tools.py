...
class RenPyTools(tk.Tk):
    ...
    def _tool_card(self, title, description, command, button_text):
        frame = tk.LabelFrame(self, text=title, padx=16, pady=12, font=("Segoe UI", 10, "bold"))
        frame.pack(fill="x", pady=7)
        tk.Label(frame, text=description, justify="left", anchor="w", wraplength=500).pack(fill="x")
        tk.Button(frame, text=button_text, command=command, width=24).pack(pady=(10, 0))
    ...
    def crear_carpetas(self):
        ...
        if creadas:
            texto = "Se creó la siguiente estructura:\n\n" + "\n".join(f"• {f}" for f in creadas)
        else:
    ...
    @staticmethod
    def _buscar_imagenes(image_folder: Path, game_folder: Path, definitions_file: Path):
        imagenes = []
        for archivo in image_folder.rglob("*"):
            ...
            nombre_imagen = " ".join(list(relativa.parts[:-1]) + [archivo.stem])
